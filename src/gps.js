...
<h1>PRUEBA 1</h1>
    ...
    <button className="btn btn-primary" onClick={()=>this.peticionPut()}><FontAwesomeIcon icon={faPen}/></button>
    <br/>
    ...
    <table className="table ">
      ...
      <tbody>
      <td>{data.tiempo !== 'Cargando...' ? data.tiempo.toFixed(2) + 'Min' : data.tiempo }</td>
      <td>{data.distancia !== 'Cargando...' ? data.distancia.toFixed(2) + 'Km' : data.distancia }</td>
      <td>{data.envio !== 'Cargando...' ? '$' + data.envio : data.envio }</td>
      </tbody>
    </table>
    <button className="btn btn-success" style={{width: '98%'}} onClick={()=>this.peticionPost()} >
                    Actualizar | Pronto
                  </button>
