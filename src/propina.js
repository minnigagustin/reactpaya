    ...
    axios.post(url, dato, headers).then((response) => {
      ...
      for (const jobNumberArray of jobsNumbersSplit) {
        ...
        axios.post(urldato, datowallet, headers).then((response) => {
          ...
          this.setState({ data: resultadaWithFleetName, dataBackup: resulta });
    ...
    axios.post(urlprofile, datoprofile, headers).then((response) => {
      ...
      this.setState({
        ...
        noti: {
          id: empresa.fleet_id,
          nombre:
            response.data.data.fleet_details[0].first_name +
            " " +
            response.data.data.fleet_details[0].last_name,
          total: (
            empresa.custom_field[11].data -
            (empresa.custom_field[11].data * 3.5) / 100
          ).toFixed(2),
        },
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        cell: (row) => (
          <div>
            {row.custom_field[0].data
              ? row.custom_field[0].data
              : row.custom_field[1].data}
          </div>
        ),
      },
      {
